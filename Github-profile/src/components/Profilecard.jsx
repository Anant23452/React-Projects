...
export default function Profilecard({ user }) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto my-6 bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 transition-all duration-300 hover:shadow-2xl">
      {/* Top Accent Bar */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-6 md:p-8">
        ...
        <div className="md:col-span-1 flex flex-col items-center text-center border-b md:border-b-0 md:border-r border-slate-100 pb-6 md:pb-0 md:pr-8">
          ...
          <h1 className="text-2xl font-bold text-gray-700 mt-4 leading-tight">
            {user.name || user.login}
          </h1>
          <p className="text-sm font-medium text-blue-600 mb-4">@{user.login}</p>
          
          {user.company && (
            <div className="mt-2 text-sm text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 w-full max-w-[200px]">
              <span className="font-semibold block text-slate-400 text-xs uppercase tracking-wider mb-0.5">Company</span>
        ...
        <div className="md:col-span-2 flex flex-col justify-between space-y-6">
          ...
          {/* Bio Section */}
          <div>
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Bio</h3>
            {user.bio ? (
              <p className="text-slate-600 leading-relaxed text-sm md:text-base">{user.bio}</p>
            ) : (
              <p className="text-slate-400 italic text-sm">This user hasn't added a bio yet.</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
            <div className="text-center p-2">
              <span className="block text-xl font-bold text-slate-800">{user.followers}</span>
              <span className="text-xs font-medium text-slate-500">Followers</span>
            </div>
            
            <div className="text-center p-2">
              <span className="block text-xl font-bold text-slate-800">{user.following}</span>
            ...
            <div className="text-center p-2 bg-blue-50 border border-blue-100 rounded-lg">
              <span className="block text-xl font-bold text-blue-600">{user.public_repos}</span>
              <span className="text-xs font-semibold text-blue-500">Public Repos</span>
            </div>
          </div>
